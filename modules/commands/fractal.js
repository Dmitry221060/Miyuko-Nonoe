...
module.exports = class FractalCommand {
    constructor(client) {
        Object.assign(this, {
            ...
            group: 'Рандом',
            description: 'Вы задаёте три параметра: Количество точек, делитель суммы координат, массив координат аттракторов(вершин). ' +
    ...
    async parseParam(msg) {
        try {
            ...
            if (isNaN(parseFloat(param[0])) || !isFinite(param[0]) || param[0] % 1 !== 0 || param[0] < 1 || param[0] > 400000) return msg.channel.send("dotCount должен быть целым числом в интервале от 1 до 400000");
            ...
            for (let i = 0; i < param.length; i++) {
                ...
                if (+param[i][0] < 0 || +param[i][1] < 0) return msg.channel.send("attractors должен содержать только числа больше 0");
            }
